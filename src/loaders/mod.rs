...
pub(crate) async fn load_files() -> bool{
    ...
    for path in paths {
        ...
        match p.is_file() {
            true => handle_file(p).unwrap(),
            false => {
                eprintln!("UNIPLEMENTED: folders and symlinks")
            }
        }
        println!();
    }
...
impl SupportedFileType {
    fn identify_filetype(file: &fs::File) -> Result<Self, String> {
        match zip::ZipArchive::new(file) {
            Ok(mut z) => match z.by_name("mimetype") {
                Ok(mut t) => {
                    ...
                    match buf_str.as_str() {
                        "application/x-krita" => return Ok(SupportedFileType::Kra),
                        "application/vnd.oasis.opendocument.text" => return Ok(SupportedFileType::Odt),
                        &_ => {}
                    }
                }
